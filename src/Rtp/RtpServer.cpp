...
public:
    ...
    RtcpHelper(Socket::Ptr rtcp_sock, MediaTuple tuple, std::string process_name) {
        _rtcp_sock = std::move(rtcp_sock);
        _tuple = std::move(tuple);
        _process_name = std::move(process_name);
    }
...
void RtpServer::start(uint16_t local_port, const char *local_ip, const MediaTuple &tuple, TcpMode tcp_mode, bool re_use_port, uint32_t ssrc, int only_track, bool multiplex, const std::string &process_name) {
    ...
    if (!tuple.stream.empty() && !multiplex) {
        ...
        helper = std::make_shared<RtcpHelper>(std::move(rtcp_socket), tuple, process_name);
        helper->startRtcp();
        ...
        _ssrc = ssrc_ptr;
        rtp_socket->setOnRead([rtp_socket, helper, ssrc_ptr, check_ssrc, bind_peer_addr](const Buffer::Ptr &buf, struct sockaddr *addr, int addr_len) mutable {
            RtpHeader *header = (RtpHeader *)buf->data();
            auto rtp_ssrc = ntohl(header->ssrc);
            auto ssrc = *ssrc_ptr;
            if (check_ssrc && ssrc && rtp_ssrc != ssrc) {
