...
public:
    ...
    RtcpHelper(Socket::Ptr rtcp_sock, std::string stream_id, std::string process_name) {
    ...
    void onRecvRtp(const Socket::Ptr &sock, const Buffer::Ptr &buf, struct sockaddr *addr) {
        if (!_process) {
            ...
            if (0 == strcasecmp(_process_name.c_str(), "GB28181")) {
                _process->setProcess(std::make_shared<GB28181Process>(_process->getMediaInfo(), _process.get()));
            } else {
