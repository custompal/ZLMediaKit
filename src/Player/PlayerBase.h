...
public:
    ...
    /**
     * 设置刷新接收总字节数回调
     * Set flush recv total bytes callback


     * [AUTO-TRANSLATED:8fb31d43]
     */
    virtual void setOnFlushRecvTotalBytes(const std::function<void(const std::string &, size_t)> &cb) = 0;

protected:
    virtual void onResume() = 0;
    virtual void onShutdown(const toolkit::SockException &ex) = 0;
    virtual void onPlayResult(const toolkit::SockException &ex) = 0;
    virtual void onFlushRecvTotalBytes(const std::string &tag, size_t bytes) = 0;
};
...
public:
    ...
    void setOnFlushRecvTotalBytes(const std::function<void(const std::string &, size_t)> &cb) override {
        if (_delegate) {
            _delegate->setOnFlushRecvTotalBytes(cb);
        }
        _on_flush_bytes = cb;
    }
...
protected:
    ...
    void onFlushRecvTotalBytes(const std::string &tag, size_t bytes) override {
        if (_on_flush_bytes) {
